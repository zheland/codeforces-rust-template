...
pub fn test_with_interactor<F1, F2>(problem: F1, interactor: F2, io_timeout: Duration)
where
    ...
    F2: FnOnce(&mut Io<BufReader<ChannelReader>, BufWriter<ChannelWriter>>) + Send + Sync,
{
    ...
    let problem_reader = BufReader::new(ChannelReader::new(recv, io_timeout));
    let interactor_writer = BufWriter::new(ChannelWriter::new(send));
    let (send, recv) = channel();
    let interactor_reader = BufReader::new(ChannelReader::new(recv, io_timeout));
