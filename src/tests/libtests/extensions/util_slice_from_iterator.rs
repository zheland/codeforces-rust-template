...
fn test_slice_from_iterator() {
    ...
    assert_eq!(array.set_from_iter(vec![5].into_iter()), 1);
    assert_eq!(array, [5, 2, 3, 4]);
    assert_eq!(array.set_from_iter(vec![6, 7].into_iter()), 2);
    assert_eq!(array, [6, 7, 3, 4]);
    assert_eq!(array.set_from_iter(vec![7, 8, 9].into_iter()), 3);
    assert_eq!(array, [7, 8, 9, 4]);
    assert_eq!(array.set_from_iter(vec![8, 9, 10, 11].into_iter()), 4);
